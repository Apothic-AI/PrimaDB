...
use bytes::Bytes;
use moq_native_ietf_draft07::quic;
use moq_transport_draft07::coding::Tuple;
use moq_transport_draft07::serve::{TrackReader, TrackReaderMode, Tracks};
...
pub struct NativeDraft07MoqRouteClient {
...
impl NativeDraft07MoqRouteClient {
    ...
    pub fn send_route(&self, route: RouteEnvelope) -> Result<()> {
        if self.closed.load(Ordering::SeqCst) {
            return Err(PrimadbError::Message(
                "native draft-07 MoQ route client is closed".to_owned(),
            ));
        }
        self.outbound
            .try_send(route)
...
impl Drop for NativeDraft07MoqRouteClient {
    fn drop(&mut self) {
        self.closed.store(true, Ordering::SeqCst);
        self.connected.store(false, Ordering::SeqCst);
        self.outbound.close();
...
async fn run_draft07_route_client(
    config: MoqRelayClientConfig,
...
async fn run_draft07_route_session(
    config: &MoqRelayClientConfig,
    outbound: Receiver<RouteEnvelope>,
    inbound: Sender<RouteEnvelope>,
    connected: Arc<AtomicBool>,
    ...
    let mut tls_args = moq_native_ietf_draft07::tls::Args::default();
    ...
    let endpoint = quic::Endpoint::new(quic::Config { bind, tls })
        .map_err(|error| PrimadbError::Message(error.to_string()))?;

    let webtransport = endpoint
        .client
        .connect(&url)
        .await
        .map_err(|error| PrimadbError::Message(error.to_string()))?;
    let (session, mut publisher, subscriber) =
        moq_transport_draft07::session::Session::connect(webtransport)
            .await
            .map_err(|error| PrimadbError::Message(error.to_string()))?;
    ...
    let track = writer
        .create(&config.track)
        .ok_or_else(|| PrimadbError::Message("draft-07 MoQ route tracks closed".to_owned()))?;
    let subgroups = track
        .groups()
        .map_err(|error| PrimadbError::Message(error.to_string()))?;

    let writer_closed = closed.clone();
    let writer_task = tokio::spawn(write_draft07_route_track(
        outbound,
        subgroups,
        writer_closed,
    ));
    ...
    for path in subscribe_namespaces {
        reader_tasks.push(tokio::spawn(read_draft07_route_path(
            subscriber.clone(),
            path,
            config.track.clone(),
            inbound.clone(),
...
async fn write_draft07_route_track(
    outbound: Receiver<RouteEnvelope>,
    mut track: moq_transport_draft07::serve::SubgroupsWriter,
    closed: Arc<AtomicBool>,
) -> Result<()> {
    while !closed.load(Ordering::SeqCst) {
        ...
        let frame = Draft07MoqRouteFrame {
...
async fn read_draft07_route_path(
    subscriber: moq_transport_draft07::session::Subscriber,
    path: String,
    track_name: String,
    inbound: Sender<RouteEnvelope>,
    closed: Arc<AtomicBool>,
    ...
    while !closed.load(Ordering::SeqCst) {
        ...
        let Some(track_reader) = reader.subscribe(&track_name) else {
            subscribe_task.abort();
            let _ = subscribe_task.await;
            return;
        };
        let mut read_task = tokio::spawn(read_draft07_track(
            track_reader,
            inbound.clone(),
            closed.clone(),
        ));
...
async fn read_draft07_track(
    ...
    while !closed.load(Ordering::SeqCst) {
        ...
        read_draft07_group(group, &inbound, &closed).await;
    }
}

async fn read_draft07_group(
    mut group: moq_transport_draft07::serve::SubgroupReader,
    inbound: &Sender<RouteEnvelope>,
    closed: &Arc<AtomicBool>,
) {
    while !closed.load(Ordering::SeqCst) {
        ...
        let Some(route) = decode_draft07_route_frame(&payload) else {
            continue;
        };
...
fn route_namespace(path: &str) -> Result<Tuple> {
    let trimmed = path.trim_matches('/');
    if trimmed.is_empty() {
        return Err(PrimadbError::Message(
            "draft-07 MoQ route namespace cannot be empty".to_owned(),
        ));
    }
    Ok(Tuple::from_utf8_path(trimmed))
}
...
fn decode_draft07_route_frame(frame: &[u8]) -> Option<RouteEnvelope> {
    if let Ok(frame) = serde_json::from_slice::<Draft07MoqRouteFrame>(frame) {
...
mod tests {
    ...
    #[test]
    fn draft07_moq_route_frame_decodes_wrapped_and_bare_routes() {
        ...
        assert_eq!(decode_draft07_route_frame(&encoded), Some(route.clone()));

        let bare = serde_json::to_vec(&route).unwrap();
        assert_eq!(decode_draft07_route_frame(&bare), Some(route));
    }

    #[test]
    fn draft07_moq_route_frame_rejects_wrong_frame_type() {
        let router = Router::new(RouterConfig::new("moq:test"));
        let route = router.peer_exchange(Vec::new(), RouteTarget::Broadcast, None);
        let frame = Draft07MoqRouteFrame {
        ...
        let encoded = serde_json::to_vec(&frame).unwrap();
        assert!(decode_draft07_route_frame(&encoded).is_none());
    }

    #[test]
    fn draft07_moq_route_frame_preserves_route_payload() {
        let router = Router::new(RouterConfig::new("moq:test"));
        ...
        let encoded = serde_json::to_vec(&Draft07MoqRouteFrame {
            frame_type: ROUTE_FRAME_TYPE.to_owned(),
            from: route.from.clone(),
            sent_at: 1,
            route: route.clone(),
        ...
        .unwrap();
        let decoded = decode_draft07_route_frame(&encoded).unwrap();
