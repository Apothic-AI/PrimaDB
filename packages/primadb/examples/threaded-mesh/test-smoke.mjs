...
async function ensureBuild() {
  if (existsSync(`${PACKAGE_ROOT}/dist/threads.js`)) {
    return;
  }
...
async function ensureServer() {
  ...
  const child = spawn("./examples/serve.sh", [], {
    cwd: PACKAGE_ROOT,
    stdio: "ignore",
    detached: true,
  });
  ...
  throw new Error(`Timed out waiting for package example server on port ${SERVER_PORT}`);
}
