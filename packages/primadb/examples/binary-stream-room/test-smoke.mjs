...
const ROOT_URL =
  process.env.PRIMADB_PACKAGE_BINARY_STREAM_URL ??
...
const ROOM = process.env.PRIMADB_PACKAGE_BINARY_STREAM_ROOM ?? `package-stream-${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    const sharedParams = `room=${encodeURIComponent(ROOM)}&signal=broadcast&capture=synthetic&chunkMs=250&windowMs=3000`;
    const publisherUrl = `${ROOT_URL}?${sharedParams}&name=publisher&autostart=1`;
    const receiverUrl = `${ROOT_URL}?${sharedParams}&name=receiver`;

    await receiver.goto(receiverUrl, { waitUntil: "networkidle" });
    await publisher.goto(publisherUrl, { waitUntil: "networkidle" });

    await publisher.waitForFunction(
      () => globalThis.binaryStreamDemo?.metrics.localChunks >= 3,
      { timeout: 30_000 },
    );

    await receiver.waitForFunction(
      () => globalThis.binaryStreamDemo?.metrics.remoteChunks >= 2,
      { timeout: 45_000 },
    );
    ...
    const result = await Promise.all([
      publisher.evaluate(() => ({
        ...
        localChunks: globalThis.binaryStreamDemo.metrics.localChunks,
        localBytes: globalThis.binaryStreamDemo.metrics.localBytes,
        room: globalThis.binaryStreamDemo.session.room,
      })),
      receiver.evaluate(() => ({
        mesh: document.querySelector("#mesh-status")?.textContent ?? "",
        remoteChunks: globalThis.binaryStreamDemo.metrics.remoteChunks,
        remoteBytes: globalThis.binaryStreamDemo.metrics.remoteBytes,
        remoteCards: document.querySelectorAll(".remote-card").length,
      })),
    ]);

    console.log(JSON.stringify({ room: ROOM, publisher: result[0], receiver: result[1] }, null, 2));
  } finally {
    await browser.close();
    if (server.started) {
