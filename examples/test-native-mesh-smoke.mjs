...
const ROOM = process.env.PRIMADB_NATIVE_MESH_ROOM ?? `native-mesh-${Date.now()}`;
const TITLE = process.env.PRIMADB_NATIVE_MESH_TITLE ?? `Native mesh smoke ${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({
      ...
      writer,
      waited,
      native_native_mesh_confirmed: true,
    }, null, 2));
  } finally {
    if (relay.started) {
