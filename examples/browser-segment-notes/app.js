...
const SYNC_CHANNEL = "primadb-browser-segment-notes-sync";
const SEGMENT_DB = "primadb-browser-segment-notes";
const SEGMENT_STORE = "records";
const SEGMENT_NAMESPACE = "main";
...
async function main() {
  ...
  const replicaId = `segment-${globalThis.crypto?.randomUUID?.() ?? Date.now()}`;
...
async function setupPersistence() {
  try {
    state.persistence = await state.db.enableIndexedDbSegmentPersistence(
      SEGMENT_DB,
      SEGMENT_STORE,
      SEGMENT_NAMESPACE,
      true,
    );
    state.persistenceLabel = "indexeddb-segments";
  } catch (error) {
    console.warn("Segment IndexedDB unavailable, falling back to snapshot persistence", error);
    ...
    state.persistenceLabel = "indexeddb-snapshot";
  }
...
async function ensureSeedTask() {
  ...
  const now = Date.now();
  state.listChain.set({
    title: "Reload me after adding a few tasks",
    note: "This example uses Primadb's canonical node/index segment records in IndexedDB.",
    done: false,
    archived: false,
    created_at: now,
    updated_at: now,
